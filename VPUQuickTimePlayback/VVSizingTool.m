...
+ (NSAffineTransform *) transformThatFitsRect:(NSRect)a inRect:(NSRect)b sizingMode:(VVSizingMode)m	{
	...
	NSRect				r = [VVSizingTool rectThatFitsRect:a inRect:b sizingMode:m];
	NSAffineTransform	*returnMe = [NSAffineTransform transform];
	[returnMe appendTransform:[[NSAffineTransform transform] translateXBy:-1*a.origin.x yBy:-1*a.origin.y]];
	[returnMe appendTransform:[[NSAffineTransform transform] scaleXBy:r.size.width/a.size.width yBy:r.size.height/a.size.height]];
	[returnMe appendTransform:[[NSAffineTransform transform] translateXBy:r.origin.x yBy:r.origin.y]];
	return t;
	*/
}
